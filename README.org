...
- Syntax highlighting enabled by default via [[https://highlightjs.org][highlight.js]] (script
  loaded only on pages that actually contain source blocks)
- LaTeX math supported natively: inline (=$x^2$=, =\(...\)=), display
  (=$$...$$=, =\[...\]=) and environments (=\begin{equation}...\end{equation}=,
  =align=, etc.) are rendered via [[https://www.mathjax.org][MathJax]], loaded only on pages that
  contain maths
...
You can also check this tutorial: [[https://bzg.fr/en/notes/orgy-tutorial/][Get ready for Orgy in 15 minutes]]
